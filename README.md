...
- Clone the repository for this guide to your local machine: `git clone git@github.com:kabanero-io/guide-nexus-repository-integration.git`
   - This guide references code samples in the `guide-nexus-repository-integration/code/` directory.
...
The complete example can be found in the `guide-nexus-repository-integration` repository that you cloned earlier. See the `/code/my-java-microprofile` directory and `stack.yaml` configuration file.
...
1. Ensure that the `settings.xml` file is in your Maven home directory, for example `~/.m2`. You can find an example `settings.xml` file in the `code/my-java-microprofile/image/project/` directory of your cloned guide repository.
